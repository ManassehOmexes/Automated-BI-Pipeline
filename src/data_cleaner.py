...
Handles data loading, missing value treatment, type optimization, and database operations.
"""
...
class DataCleaner:
    ...
    def load_data(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            self.logger.info(
                f"Daten geladen: {len(self.data)} Zeilen, {len(self.data.columns)} Spalten"
            )
            return self.data
            ...
            self.logger.error(
                f"Die Datei '{self.filepath}' ist leer und enthält keine Daten."
            )
            return None
        ...
        try:
            ...
            for col in self.data.columns:
                if missing_count[col] > 0:
                    self.logger.info(
                        f"{col}: {missing_count[col]} ({missing_percent[col]:.2f}%)"
                    )
            ...
            for col in numeric_cols:
                if missing_count[col] > 0:
                    ...
                    self.logger.info(
                        f"{col} (numerisch): Gefuellt mit Median ({median_value:.2f})"
                    )
            ...
            for col in categorical_cols:
                if missing_count[col] > 0:
                    if not self.data[col].mode().empty:
                        ...
                        self.logger.info(
                            f"{col} (kategorisch): Gefuellt mit Mode ('{mode_value}')"
                        )
                    else:
            ...
            self.logger.error(
                f"Fehler beim Behandeln fehlender Werte: {type(e).__name__}: {str(e)}"
            )
            return None
    ...
    def correct_datatypes(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            for col in self.data.columns:
                if "date" in col.lower() or "time" in col.lower():
                    try:
                        ...
                        self.logger.error(
                            f"'{col}' konnte nicht konvertiert werden: {e}"
                        )
            ...
            self.logger.info(
                f"Eingespart: {memory_saved:.2f} MB ({memory_percent:.1f}%)"
            )
            ...
            self.logger.error(
                f"Fehler beim Korrigieren der Datentypen: {type(e).__name__}: {str(e)}"
            )
            return None

    def save_to_database(
        self, df: pd.DataFrame, table_name: str = "cleaned_sales"
    ) -> None:
        """
